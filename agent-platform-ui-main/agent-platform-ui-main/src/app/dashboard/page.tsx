...
import { ErrorBoundary, AgentPanelErrorBoundary, ChatErrorBoundary, MetricsErrorBoundary } from "@/components/ui/ErrorBoundary"
    ...
    <ErrorBoundary fallbackMessage="Dashboard failed to load">
      <div className="flex flex-1 flex-col p-6 h-full overflow-hidden">
        ...
        <div className="grid grid-cols-4 gap-4 mb-4">
          {kpisLoading || !kpis ? (
            metrics.map((_, i) => <MetricSkeleton key={`skeleton-${i}`} index={i} />)
          ) : (
            metrics.map((metric, i) => (
              <MetricsErrorBoundary key={metric.label}>
                <MetricTile index={i} {...metric} />
