...
import { ErrorBoundary, ChatErrorBoundary } from "@/components/ui/ErrorBoundary"
import { SkeletonCard } from "@/components/ui/SkeletonCard"
import { Suspense } from "react"

export default function ChatPage() {
  return (
    <ErrorBoundary fallbackMessage="Chat page failed to load">
      <div className="flex flex-1 flex-col p-6 h-full overflow-hidden">
        ...
        <div className="flex-1 flex overflow-hidden min-h-0 rounded-xl border border-border/30 glass-card">
          <ChatErrorBoundary>
            <Suspense fallback={<div className="flex-1 flex items-center justify-center"><SkeletonCard className="h-64" /></div>}>
              <AIAssistant />
            </Suspense>
          </ChatErrorBoundary>
        </div>
